...
namespace GeoDemo
{
    public partial class SanYanShiFenLeiTwo2 : Form
    {
        struct area
        {
            public int i;//模块编号
       ...
       public SolidBrush mybrush = new SolidBrush(Color.Yellow);
        ...
        private void Form8_Paint(object sender, PaintEventArgs e)
        {
            ...
            g.DrawLine(mypen1, 310, 128, 275, 128);
            g.DrawString("石英砂岩", myfont,new SolidBrush(Color.Black), 220, 123);
            g.DrawLine(mypen1, 286, 166, 255, 166);
            g.DrawString("长石质石英砂岩", myfont, new SolidBrush(Color.Black), 160, 161);
            g.DrawLine(mypen1, 337, 166, 361, 166);
            g.DrawString("岩屑质石英砂岩", myfont, new SolidBrush(Color.Black), 365, 161);

            g.DrawLine(mypen1, 312, 219, 395, 219);
            g.DrawString("长石岩屑质石英砂岩", myfont, new SolidBrush(Color.Black), 400, 212);
            ...
            for (int i = 0; i < str.Length; i++)
            {
                px = px - 10;
                py = py + 30;
                g.DrawString(str[i], myfont, new SolidBrush(Color.Black), 240 + px, 210 + py);
            }
            string[] str1 = new string[7] { "岩", "屑", "质", "长", "石", "砂", "岩" };
            px = 0; py = 0;
            for (int i = 0; i < str1.Length; i++)
            {
                px = px - 15;
                py = py + 30;
                g.DrawString(str1[i], myfont, new SolidBrush(Color.Black), 282 + px, 248 + py);
            }
            string[] str2 = new string[6] { "岩", "屑", "长", "石", "砂", "岩" };
            px = 0; py = 0;
            for (int i = 0; i < str2.Length; i++)
            {
                // px = px + 5;
                py = py + 20;
                g.DrawString(str2[i], myfont, new SolidBrush(Color.Black), 280 + px, 315 + py);
            }

            string[] str3 = new string[6] { "长", "石", "岩", "屑", "砂", "岩" };
            px = 0; py = 0;
            for (int i = 0; i < str3.Length; i++)
            {
                // px = px + 10;
                py = py + 20;
                g.DrawString(str3[i], myfont, new SolidBrush(Color.Black), 325 + px, 315 + py);
            }

            string[] str4 = new string[7] { "长", "石", "质", "岩", "屑", "砂", "岩" };
            px = 0; py = 0;
            for (int i = 0; i < str4.Length; i++)
            {
                px = px + 10;
                py = py + 20;
                g.DrawString(str4[i], myfont, new SolidBrush(Color.Black), 323 + px, 247 + py);
            }

            string[] str5 = new string[4] { "岩", "屑", "砂", "岩" };
            px = 0; py = 0;
            for (int i = 0; i < str5.Length; i++)
            {
                px = px + 10;
                py = py + 20;
                g.DrawString(str5[i], myfont, new SolidBrush(Color.Black), 368 + px, 229 + py);
            }
        ...
        private void 岩石分类图图版2图二_Load(object sender, EventArgs e)
        {
            this.BringToFront();
            //this.Name = "岩石分类图图版2图二";
        ...
        private void 复制ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            MyObject.MyCurrentFrmName1 = "岩石分类图图版2图二";
