...
namespace GeoDemo
{
    public partial class ReadDataAll : Form
    {
        ////是否是普通图 还是直方图
        //public static bool IsCommon=true;
        //存储excel中表名
      ...
      public  bool isErrorShow = false;
        ...
        public void SetDataGridViewValue(ref DataSet dataset,string[,]WordsArray)
        {
            ...
            try
            {
                ...
                string[,] ContentArray = new string[row, col];//将第一行去掉保存在一个字符串数组中
                for (int i = 0; i < row; i++)
                {
                    for (int j = 0; j < col; j++)
                    {
                        ContentArray[i, j] = WordsArray[i, j];
                    }
                }
                for (int j = 0; j < col; j++)
            ...
            for (int i = 0; i < arr.GetLength(0); i++)
            {
                ...
                for (int j = 0; j < arr.GetLength(1); j++)
                {
                    if (i==0)
                    {
                        if (j==0)
                        {
                            DataColumn nc = new DataColumn("水平轴");
                            dataSouce.Columns.Add(nc);
                            newRow[j] = arr[i, j];
                        }
                        else
                        {
                            DataColumn nc2 = new DataColumn("序列" + j);
                            dataSouce.Columns.Add(nc2);
                            newRow[j] = arr[i, j];
                        }
                    }
                    else
        ...
        //表格绘制
        public static DataGridView dgvData = new DataGridView();
        private void DataGridViewMaker(DataSet tdS)
        {
            ...
            for (int i = 0; i < tdS.Tables.Count; i++)
            {
                ...
                source.DataSource = tdS.Tables[i];
                MyObject.Columns = tdS.Tables[i].Columns.Count;
                selectAll =new bool [MyObject .Columns];
                foreach (DataColumn  col in tdS.Tables [i].Columns)
                {
                        ...
                        dgvData.Columns.Add(commoncolumn);
                }
                ...
                dgvData.ColumnHeaderMouseClick += new DataGridViewCellMouseEventHandler(dgvData_ColumnHeaderMouseClick);
        ...
        void dgvData_SelectionChanged(object sender, EventArgs e)
        {
            ...
            if (dgvData != null)
            {
                foreach (DataGridViewTextBoxCell cell in dgvData.SelectedCells)
                {
                    if (!RowIndex.Contains(cell.RowIndex))
                    {
                        RowIndex.Add(cell.RowIndex);
                    }
                    if (!ColumnIndex.Contains(cell.ColumnIndex))
                    {
                        ColumnIndex.Add(cell.ColumnIndex);
                    }
                }
            }
            if (RowIndex.Count >0)
            {
                RowIndex.Sort();
                firstrow = RowIndex[0];
                lastrow = RowIndex[RowIndex.Count - 1];
            }
            if (ColumnIndex . Count > 0)
            {
                ColumnIndex.Sort();
                firstcolumn  = ColumnIndex[0];
                lastcolumn  = ColumnIndex[ColumnIndex.Count - 1];
            }
        }
        ...
        void dgvData_ColumnHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            {
                for (int k = 0; k < MyObject .Columns; k++)
                {
                    if (k==e.ColumnIndex)
                    {
                        dgvData.Rows[i].Cells[e.ColumnIndex].Selected = true;
                        selectAll[e.ColumnIndex] = true;
                    }
                    else
                    {
                        dgvData.Rows[i].Cells[k].Selected = false;
                        selectAll[k] = false;
                    }
                }
            }
        ...
        private void 读取数据_FormClosed(object sender, FormClosedEventArgs e)
        {
                ...
                if (MyObject.Dataset != null)
                {
                    ...
                    else
                    {
                        ...
                        //如果选择的是普通图 x轴为string类型 令DrawChart类里的X轴精度为1 
                        ...
                        if (comboBox1.SelectedIndex == 0)
                        {
                            ...
                            //MyObject.My_Chart1.ChartAreas[0].AxisX.Interval = 1;
                        }
                        else
                        {
                            //IsCommon = false;
        ...
        private void 读取数据_Load(object sender, EventArgs e)
        {
            ...
            tabControl_Data.TabPages.Clear();
            if (dgvData != null && MyObject.DT != null && MyObject.My_Chart1.Series[0].Points.Count != 0)
            {
                MyObject.Columns = MyObject.DT.Columns.Count;
                dgvData.Columns.Clear();
                dgvData.Controls.Clear();
                BindingSource source = new BindingSource();
                source.DataSource = MyObject.DT;
                foreach (DataColumn col in MyObject.DT.Columns)
                {
                    DataGridViewAutoFilterTextBoxColumn commoncolumn = new DataGridViewAutoFilterTextBoxColumn();
                    commoncolumn.DataPropertyName = col.ColumnName;
                    commoncolumn.HeaderText = col.ColumnName;
        ...
        private void btnApply_Click(object sender, EventArgs e)//应用
        {
            for (int i = 0; i < dgvData.Rows.Count - 1; i++)
            {
                for (int j = 0; j < dgvData.Columns.Count; j++)
                {
                    ...
                    if (dgvData.Rows[i].Cells[j].Value.ToString() == "")
                    {
                        MessageBox.Show("数据读取有误，请仔细检查数据！");
                        return;
                    }
            ...
            for (int i = 1; i < dgvData.Rows.Count - 1; i++)
            {
                ...
                {
                    if (isNumberic(dgvData.Rows[i].Cells[j].Value.ToString()))
                    {
                        MessageBox.Show("数据读取有误，请仔细检查数据！");
                        return;
                    }
        ...
        public static bool isDateTime(string message)
        {
            //判断是否为数字
            //是的话则返回false, 否则为true
            try
            {
                DateTime result;
                result = Convert.ToDateTime(message);
                return false;
            }
            catch
            {
        ...
        {
            for (int i = dgvData .SelectedRows.Count; i >0; i--)
            {
                dgvData.Rows.RemoveAt(dgvData .SelectedRows [i-1].Index);
            }
        }
        ...
        {
            int p = 0;
            for (int i = 0; i < MyObject .Columns ; i++)
            {
                if (selectAll [i])
                {
        ...
        private void 添加序列ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            MyObject.AddSeries = true;
            MyObject.Columns++;
            DataGridViewAutoFilterTextBoxColumn commoncolumn = new DataGridViewAutoFilterTextBoxColumn();
            if (MyObject .Columns ==0)
            {
                commoncolumn.HeaderText = "水平轴";
            }
            else
            {
                commoncolumn.HeaderText = "序列" + (MyObject.Columns-1);
            }
            commoncolumn.Resizable = DataGridViewTriState.True;
            dgvData.Columns.Add(commoncolumn);
            dgvData.Width = tabControl_Data.Width - 10;
        ...
        private void 插入数据ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < MyObject.Columns; i++)
                {
                    if (selectAll[i])
                    {
                        for (int k = 0; k < dgvData .RowCount-1; k++)
                        {
                            dgvData.Rows[k].Cells[i].Value = data[k, 0].ToString();
                        }
                        o++;
                    }
                }
                if (o==0)
                {
                    for (int n = 0; n < lastrow - firstrow + 1; n++)
                    {
                        for (int m = 0; m < lastcolumn - firstcolumn + 1; m++)
                        {
                            if (data[n, m] != null)
                            {
                                dgvData.Rows[firstrow + n].Cells[firstcolumn + m].Value = data[n, m].ToString();
                            }
                        }
                    }
                }
                //if (MyObject.DT.Columns.Count != MyObject.Columns)
                //{
                //    for (int k = 0; k < MyObject.Columns - MyObject.DT.Columns.Count; k++)
                //    {
                //        MyObject.DT.Columns.Add("序列" + MyObject.DT.Columns.Count);
                //    }
                //}

                //for (int b = 0; b < dgvData.Rows.Count - 1; b++)
                //{
                //    MyObject.DT.Rows.RemoveAt(b);
                //    DataRow dr = MyObject.DT.NewRow();
                //    for (int c = 0; c < MyObject.Columns; c++)
                //    {
                //        dr[c] = dgvData.Rows[b].Cells[c].Value.ToString();
                //    }
        ...
        {
            MyObject.O = 0;
            MyObject.U = dgvData.RowCount - 2;
            int t = 0;
            foreach (DataGridViewAutoFilterTextBoxColumn col in dgvData.Columns)
            {
            ...
            if (t == 0)
            {
                MessageBox.Show("请不要忘了绑定水平轴的数据");
                return;
            }
            MyObject.O = MyObject.Columns - MyObject.O;
            string[] x1 = new string[dgvData.RowCount - 2];
            double[] x2 = new double[dgvData.RowCount - 2];
            
            if (MyObject.O != 0)
            {
                double[,] y = new double[MyObject.O, dgvData.RowCount - 2];
                string[] z = new string[MyObject.O + 1];
                ReadDataFromDatatable readdatafromdatatable = new ReadDataFromDatatable();
                readdatafromdatatable.readDataService(ref x1, ref y, ref z);
                if (MyObject.My_Chart1.Series.Count > MyObject.O)
                {
                    for (int ik = 0; ik < MyObject.My_Chart1.Series.Count - MyObject.O; ik++)
                    {
                        MyObject.My_Chart1.Series.RemoveAt(ik + MyObject.O);
                    }
                }
                   for (int i = 0; i < MyObject.O; i++)
                   {
                       ...
                       {
                           double[] d = new double[dgvData.RowCount - 2];
                           for (int q = 0; q < dgvData.RowCount - 2; q++)
                           {
                               d[q] = y[i, q];
                           }
                       ...
                       else
                       {
                           ...
                           MyObject.My_Chart1.Series.Add(series);
                           //把该图形设为浮雕形
                           if (MyObject.My_Chart1.Series[0]["DrawingStyle"] == "Emboss")
                           {
                               MyObject.My_Chart1.Series[series.Name]["DrawingStyle"] = "Emboss";//绘图风格默认的是方形，此时可以改变成圆柱型
                           }
                           else
                           {
                               MyObject.My_Chart1.Series[series.Name]["DrawingStyle"] = "Cylinder";
                           }
                           ...
                           series.ChartType = MyObject.My_Chart1.Series[0].ChartType;//让新加的序列的类型与原类型一样
                           series["PointWidth"] = "0.6";
                           double[] b = new double[dgvData.RowCount - 2];
                           for (int q = 0; q < dgvData.RowCount - 2; q++)
                           {
                               b[q] = y[i, q];
                           }
                           MyObject.My_Chart1.Series[i].Points.Clear();
                           MyObject.My_Chart1.Series[i].Points.DataBindXY(x1, b);
                           MyObject.My_Chart1.Series[i].Name = z[i + 1];
                       }
                   }
                //}
                ...
                //{
                //    readdatafromdatatable.readDataService(ref x2, ref y, ref z);
                //    for (int i = 0; i < MyObject.O; i++)
                //    {
                //        if (MyObject.My_Chart1.Series.Count > i)
                //        {
                ...
                //                d[q] = y[i, q];
                //            }
                //            MyObject.My_Chart1.Series[i].Points.DataBindXY(x2, d);
                //            MyObject.My_Chart1.Series[i].Name = z[i + 1];
                //        }
                //        else
                //        {
                ...
                //                b[q] = y[i, q];
                //            }
                //            MyObject.My_Chart1.Series[i].Points.DataBindXY(x2, b);
                //            MyObject.My_Chart1.Series[i].Name = z[i + 1];
                //        }
                //    }
                //}
                ...
                if (z[0] != "")
                {
                    MyObject.My_Chart1.ChartAreas[0].AxisX.Title = z[0];
                }
